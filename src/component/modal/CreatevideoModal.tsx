...
import { FaCalendarWeek, FaClock, FaCopy, FaDotCircle, FaKey, FaLock } from "react-icons/fa";

const CreateVideoModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  const [step, setStep] = useState(2);

  // Form states (can later be lifted or saved in context)
  const [surveyType] = useState("video");
      ...
      >
        ...
        <AnimatePresence mode="wait">
          {/*  {step === 1 && (
            ...
            >
              <p className="text-white/70 text-sm">Choose your survey type:</p>
          ...
          {step === 3 && (
            <motion.div key="step3" className="mt-6 space-y-6">
                ...
                {showValidity && (
                  <div className="px-4 pb-4 space-y-4">
                    ...
                    <div>
                      ...
                      <div className="flex  flex-wrap items-center gap-2  bg-white/7 border border-white/10">
                        <input
                          type="date"
                          value={validFromDate}
                          onChange={(e) => setValidFromDate(e.target.value)}
                          className="flex-1 uppercase rounded-md text-white px-3 py-2 text-sm"
                        />
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap  items-center gap-2  bg-white/7 border border-white/10">
                        <input
                          ...
                          className="flex-1 rounded-md text-white px-3 py-2 text-sm"
                        />
                        <button className="scale-90 border border-blue-400/20 text-blue-400 text-xs px-3 py-2 rounded-md">
                          Create Schedule
                        </button>
              ...
              <p className="font-medium w-fit bg-white/15 text-xs rounded p-1 px-4">
                Meeting Room Link
              </p>

              <div className="-mt-3">
                <h3 className="block text-xs text-white/50 mb-1">
                  General Room Link
                </h3>
                <p className="flex  p-3 text-blue-400 flex-wrap items-center gap-2  bg-white/7 border border-white/10">
                  https://surdatics.com/video-survey/safety-compliance
                </p>
                <button className="flex items-center mt-2 gap-2 scale-90 border border-blue-400/20 text-blue-400 text-xs px-6 py-2 rounded-md">
                  <FaArrowRotateRight /> Generate Link
                </button>
              </div>
            </motion.div>
          )}
          ...
          {step === 4 && (
            <motion.div key="step4" className="mt-6 space-y-6">
              {/* Survey Validity */}
              <div className="bg-black/30 rounded-lg border border-white/10">
                <button className="w-full flex justify-between items-center px-4 py-3 text-white/70 text-sm">
            ...
            <motion.div key="step5" className="mt-6 space-y-6">
              <div>
                ...
                <p className="text-xs text-white/40">
                  ({Number(budget || 0).toLocaleString()} Surda)
                </p>
              </div>
                    ...
                    <div className="space-y-4 ">
                      ...
                      <div>
                        ...
                        <div className=" flex items-center gap-3">
                          ...
                          <p className=" bg-white/5 border border-white/10 rounded-full px-3 py-2 text-white/60 text-sm flex gap-2 items-center">
                            <FaDotCircle size={10} /> <FaClock /> {validToTime}
                          </p>
                        </div>
                      </div>
                    </div>
